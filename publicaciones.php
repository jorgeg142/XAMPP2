...
$TotalReg = mysqli_query($connect, "SELECT * FROM publicaciones");
...
while ($lista = mysqli_fetch_array($consulta)) {
    ...
    $fotos = mysqli_query($connect, "SELECT * FROM fotos WHERE publicacion = '$lista[id_pub]'");
...
<div class="box box-widget">
    ...
    <div class="box-body">
        ...
        <?php if ($lista['imagen'] != 0) { ?>
            <img src="publicaciones/<?php echo $fot['ruta']; ?>" width="50%">
        <?php } ?>
        <br><br>
        <?php $numcomen = mysqli_num_rows(mysqli_query($connect, "SELECT * FROM comentarios WHERE publicacion = '" . $lista['id_pub'] . "'")); ?>
        <ul class="list-inline">
            <?php
            $query = mysqli_query($connect, "SELECT * FROM likes WHERE post = '" . $lista['id_pub'] . "' AND usuario ='" . $_SESSION['id'] . "'");
            ...
            <?php } ?>
            <li class="pull-right">
                <span class="link-black text-sm"><i class="fa fa-comments-o margin-r-5"></i> Comentarios (<?php echo $numcomen; ?>)</span>
            </li>
        </ul>
    </div>
    <div class="box-footer box-comments">
        ...
        while ($com = mysqli_fetch_array($comentarios)) {
            $usuarioc = mysqli_query($connect, "SELECT * FROM usuarios WHERE id_use = '" . $com['usuario'] . "'");
